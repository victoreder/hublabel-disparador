...
export function buildSystemPrompt(job, agente, { handoff = false } = {}) {
  ...
  const partes = [
    ...
    '- Mídias vindas do CONHECIMENTO RECUPERADO podem e devem ser enviadas quando ajudarem a responder sobre o produto.',
    '- Formato markdown: imagem [nome (image)](https://...), vídeo [nome (video)](https://...), áudio [nome (audio)](https://...). Nunca altere a URL; separe mídias e textos com 2 enters.',
    ...
    '- O retorno da ferramenta consultar_conhecimento é uma fonte interna; nunca exponha JSON, metadata, registros, documentos, resultados ou detalhes da consulta.',
    '- Converta os dados recuperados em uma resposta natural de atendimento. Não use listas de campos como Nome:, Descrição: e Preço: nem comece com “Encontrei estes registros”.',
    '- Prefira respostas curtas e conversacionais. Se existirem opções diferentes ou informações conflitantes, explique isso naturalmente e pergunte qual opção interessa.',
    '- Ao responder sobre vários produtos com mídia, mantenha a ordem: texto do produto 1, mídia do produto 1, texto do produto 2, mídia do produto 2. Nunca deixe todas as fotos para o final.',
    '',
    '## INSTRUÇÕES:',
    agente?.instrucoes || '',
    '-----',
