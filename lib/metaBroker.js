...
function getConfig() {
  ...
  const WEBHOOK_BACK_PREFIX = process.env.META_WEBHOOK_BACK_PREFIX || "back";
  const WEBHOOK_EVENTS_PATH =
    process.env.META_WEBHOOK_EVENTS_PATH || "/eventsmeta";
  const META_WEBHOOK_VERIFY_TOKEN =
    process.env.META_WEBHOOK_VERIFY_TOKEN || STATE_SECRET;
...
function extractRootDomain(hostname) {
  ...
  for (const suffix of BR_REGISTRABLE_SUFFIXES) {
...
function buildInboundWebhookUrl(parsedReturn, config) {
  const hostname = new URL(parsedReturn.origin).hostname;
  const rootDomain = extractRootDomain(hostname);
  const prefix = config.WEBHOOK_BACK_PREFIX || "back";
  const eventsPath = config.WEBHOOK_EVENTS_PATH || "/eventsmeta";
  const normalizedPath = eventsPath.startsWith("/")
    ? eventsPath
    : `/${eventsPath}`;

  const callbackUrl = `https://${prefix}.${rootDomain}${normalizedPath}`;
  if (callbackUrl.length > 200) {
    throw new Error(
      "URL de webhook derivada excede 200 caracteres (limite da Meta)."
    );
  }
  return callbackUrl;
}
...
async function graphPost(path, accessToken, body) {
  ...
  const data = await response.json();
  if (!response.ok || data.error) {
    throw new Error(data?.error?.message || "Erro na Graph API POST");
  }
  return data;
}
...
async function setPhoneWebhookOverride({
  phoneNumberId,
  accessToken,
  callbackUri,
  verifyToken
}) {
  return graphPost(`/${phoneNumberId}`, accessToken, {
...
async function completeMetaConnection({
  ...
  const inboundWebhookUrl =
    typeof parsedState.inbound_webhook_url === "string"
      ? parsedState.inbound_webhook_url
      : buildInboundWebhookUrl(
          {
            origin: returnOrigin,
            basePath: parsedState.return_base_path || ""
          },
          config
        );

  try {
    await configureConnectionWebhook({
      wabaId: assets.waba_id,
      phoneNumberId: assets.phone_number_id,
      accessToken: longToken.access_token,
      callbackUri: inboundWebhookUrl,
    ...
    return {
      ...
      donePageOptions,
      error: {
        reason: "webhook_override_failed",
        inbound_webhook_url: inboundWebhookUrl,
        error: err instanceof Error ? err.message : "Falha ao configurar webhook"
      }
    };
  }
  ...
  const payload = {
    ...
    inbound_webhook_url: inboundWebhookUrl,
    embeddedSignupEvent: sessionHints?.embeddedSignupEvent || null,
    flowType: sessionHints?.flowType || null
  };
      ...
      window.addEventListener("message", function (event) {
        ...
        try {
          ...
          if (
            ...
            sessionData = {
              ...payload.data,
              embeddedSignupEvent: payload.event,
              flowType:
                payload.event === "FINISH_WHATSAPP_BUSINESS_APP_ONBOARDING"
                  ? "coexistence"
                  : "standard"
